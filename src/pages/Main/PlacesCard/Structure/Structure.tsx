...
export const Structure: React.FC<StructureType> = ({
  data,
  selected,
  selectAction,
}) => {
  ...
  const getInventoryInfo = (id: string) =>
    list.filter((item) => item.placeId === id).length;
  ...
  const renderLevel = (source: PlaceType[], level = 0) => {
    ...
    return source.map((item) => (
      <Level key={item.id} level={level}>
        ...
        >
          {item.name}
          <Badge count={getInventoryInfo(item.id)} />
        </Item>
  ...
  return <Container>{renderLevel(data)}</Container>;
};
